var app = new Vue({
    ...
    data: {
        ...
        bitcoin: {
            ...
            faucets: [
                ...
                ["thonguyen", "https://faucet.thonguyen.net/btc"]
            ],
            price: 0,
            symbol: "BTC",
            tx: []
        ...
        litecoin: {
            ...
            faucets: [
                ...
                ["thonguyen", "https://faucet.thonguyen.net/ltc"]
            ],
            price: 0,
            symbol: "LTC",
            tx: []
        ...
        dash: {
            ...
            color: "green",
            faucets: [
                ["masternode", "http://test.faucet.masternode.io"],
                ["thonguyen", "https://faucet.thonguyen.net/dash"]
            ],
            price: 0,
            symbol: "DASH",
            tx: []
    ...
    methods: {
        ...
        checkKey: function (priv_key) {
            ...
            } else if (this.current == "dash") {
                network_v = blt.networks.dashtestnet;
            }
            var keyPair = blt.ECPair.fromWIF(priv_key, network_v);
            ...
            if (window.location.hash[1] == "b") {
                ...
                if (isValidAddress(address, 'dash')) {
                    this.dash.address = address;
                    this.current = "dash";
                    return true;
                }
            }
        ...
        sendTransaction: function () {
            ...
            if (isValidAddress(recipient_address, this.current)) {
                if (this[this.current].amount > 0 && send_amount < this[this.current].amount) {
                    ...
                    var keyPair = blt.ECPair.fromWIF(rot13(window.location.hash.match(/(b|l|d)\-([a-zA-Z0-9]+)(-([A-Z]{3}))?/)[2]), network_v);
                    var tx = new blt.TransactionBuilder(network_v);

                    app.getUnspentTransactions(send_amount, tx, keyPair).done(function (res) {
        ...
        switchCurrency: function () {
            var coin = "bitcoin";
            if (this.current == "bitcoin") {
                coin = "litecoin"
            } else if (this.current == "litecoin") {
                coin = "dash"
            }
            this.init(coin);
        },
        ...
        updatePrices: function () {
            return $.get("https://min-api.cryptocompare.com/data/pricemulti?fsyms=BTC,LTC,DASH&tsyms=" + this.currentFiat).done(function (res) {
                app.bitcoin.price = res['BTC'][app.currentFiat];
                app.litecoin.price = res['LTC'][app.currentFiat];
                app.dash.price = res['DASH'][app.currentFiat];
            });
        },
    ...
    computed: {
        ...
        baseURL: function () {
            var base_url = "https://test-insight.bitpay.com";
            if (app.symbol === "LTC") {
                base_url = "https://testnet.litecore.io";
            } else if (app.symbol === "DASH") {
                base_url = "https://test.insight.dash.siampm.com";
            }
            return base_url
        },
        color: function () {
...
function isValidAddress(address, network) {
    try {
        if (network == "bitcoin") {
            ...
            return blt.address.toOutputScript(address, blt.networks.dashtestnet);
        }
    } catch (err) {
        return false;
    }
...
function createNewAddress(network) {
    ...
    var keyPair = blt.ECPair.makeRandom({network: network_v});
...
$(document).ready(function () {
    ...
    if (/(b|l|d)\-([a-zA-Z0-9]+)(-([A-Z]{3}))?/.test(window.location.hash)) {
        // set currency
        var user_currency = window.location.hash.match(/(b|l|d)\-([a-zA-Z0-9]+)(-([A-Z]{3}))?/)[4];
        if (user_currency in app.currencies) {
            app.currentFiat = user_currency;
        }
        try {
            if (app.checkKey(rot13(window.location.hash.match(/(b|l|d)\-([a-zA-Z0-9]+)(-([A-Z]{3}))?/)[2]))) {
                app.updateData();
            }
        } catch (err) {
            alert(err);
